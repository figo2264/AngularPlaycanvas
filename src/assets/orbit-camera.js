...
OrbitCamera.attributes.add('inertiaFactor', {
  ...
  description: 'Higher value means that the scripts will continue moving after the user has stopped dragging. 0 is fully responsive.'
});

OrbitCamera.attributes.add('focusEntity', {
  type: 'entity',
  title: 'Focus Entity',
  description: 'Entity for the scripts to focus on. If blank, then the scripts will use the whole scene'
});
...
// Property to get and set the distance between the pivot point and scripts
...
// Property to get and set the pitch of the scripts around the pivot point (degrees)
// Clamped between this.pitchAngleMin and this.pitchAngleMax
// When set at 0, the scripts angle is flat, looking along the horizon
...
// Property to get and set the yaw of the scripts around the pivot point (degrees)
...
// Property to get and set the world position of the pivot point that the scripts orbits around
...
// Moves the scripts to look at an entity and all its children so they are all in the view
OrbitCamera.prototype.focus = function (focusEntity) {
  // Calculate an bounding box that encompasses all the models to frame in the scripts view
...
// Set the scripts position to a world position and look at a world position
...
// Set scripts position to a world position and look at an entity in the scene
...
// Set the scripts at a specific, yaw, pitch and distance without inertia (instant cut)
...
OrbitCamera.prototype.initialize = function () {
  ...
  // Calculate the scripts euler angle rotation around x and y axes
  // This allows us to place the scripts at a particular rotation to begin with in the scene
  var cameraQuat = this.entity.getRotation();
  ...
  // If we have ticked focus on start, then attempt to position the scripts where it frames
  // the focused entity and move the pivot point to entity's position otherwise, set the distance
  // to be between the scripts position in the scene and the pivot point
...
OrbitCamera.prototype._updatePosition = function () {
  // Work out the scripts position based on the pivot point, pitch, yaw and distance
...
OrbitCamera.prototype._clampPitchAngle = function (pitch) {
  // Negative due as the pitch is inversed since the scripts is orbiting the entity
...
OrbitCameraInputMouse.attributes.add('orbitSensitivity', {
  ...
  description: 'How fast the scripts moves around the orbit. Higher is faster'
});
...
OrbitCameraInputMouse.attributes.add('distanceSensitivity', {
  ...
  description: 'How fast the scripts moves in and out. Higher is faster'
});
...
OrbitCameraInputMouse.prototype.onMouseMove = function (event) {
  const orbitCamera = pc.app.root.findByName('orbitCameraEntity');
  const viewSizeRatio = orbitCamera.camera.scissorRect.z;
  if( event.x < this.app.graphicsDevice.canvas.clientWidth * viewSizeRatio &&
    event.y < this.app.graphicsDevice.canvas.clientHeight * viewSizeRatio) {
...
OrbitCameraInputMouse.prototype.onMouseWheel = function (event) {
  this.orbitCamera.sliderUse = false;
...
OrbitCameraInputTouch.prototype.onTouchStartEndCancel = function (event) {
  // We only care about the first touch for scripts rotation. As the user touches the screen,
...
OrbitCameraInputTouch.prototype.onTouchMove = function (event) {
  ...
  // We only care about the first touch for scripts rotation. Work out the difference moved since the last event
  // and use that to update the scripts target position
  var touches = event.touches;
  if (touches.length == 1) {
    var touch = touches[0];
